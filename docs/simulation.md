...
Health and modeled damage affect pace and incident exposure. Pit servicing repairs the scalar damage model, not an itemized parts inventory. The previous HTML engine's separate aero/suspension/brakes/gearbox components, wear histories, stress system and finite tyre-set allocations are not present here.
...
Qualifying uses measured flying-lap crossings. Race sector ends use authored sector gates where exactly two valid boundaries exist, otherwise thirds. Classification and finish processing are resolved after every car's movement in the tick. This avoids awarding a close finish to whichever driver happened to appear first in the roster array.

## Performance boundaries

Track compilation is synchronous and bounded. The maximum-size authoring document is a validation ceiling, not a guarantee of smooth interactive editing on every computer. Normal bundled layouts use 14–250 control points. Track drawing is retained separately from moving cars, and text refreshes at 5 Hz. A native profiler pass on target hardware remains appropriate before claiming a frame-rate guarantee.
