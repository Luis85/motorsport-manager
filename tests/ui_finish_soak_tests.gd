...
func run() -> void:
	...
	while (Time.get_ticks_msec()-start)/1000.0<seconds:
		...
		if elapsed>=next_action:
			...
			observations.append({"wall_seconds":elapsed,"race_seconds":model.total_time,"phase":model.phase,"nodes":get_node_count(),"race_styles":PitwallDesign.race_styles.size(),"static_memory_bytes":Performance.get_monitor(Performance.MEMORY_STATIC)})
			view.close_detail();await settle();await key(KEY_SPACE);route+=1;next_action=elapsed+15.0
	...
	var report={"passed":failures.is_empty(),"checks":checks,"errors":failures,"captures":captures,"screenshots":captures.size(),"engine":Engine.get_version_info().string,"cpu":OS.get_processor_name(),"renderer":RenderingServer.get_video_adapter_name(),"display":DisplayServer.get_name(),"viewport":"1100x720","text_scale":1.3,"wall_seconds":(Time.get_ticks_msec()-start)/1000.0,"target_wall_seconds":seconds,"frames":frame,"frame_sample_every":6,"frame_distribution":quantiles(frame_samples),"baseline_nodes":baseline_nodes,"baseline_static_memory_bytes":baseline_memory,"observations":observations,"completed_physical_sessions":session_results,"final_race_time":model.total_time,"final_hash":race_fingerprint().sha256_text(),"limitations":"Native Linux Xvfb / Mesa llvmpipe, 1x real frame-loop input with explicit pause inspections. Contending CI/test workloads may affect throughput. Not physical-controller, Windows, export-build or broad-hardware certification; not a deterministic fixed-delta throughput comparison."}
