...
func run() -> void:
	...
	# Details is now an integrated non-modal drawer, not a separate AcceptDialog.
	check(view.tabs.current_tab == view.decision_page_index and view.decision_drawer.evidence.text.contains(view.decision_controls[6].battle.text), "Named Details action preserves full battle status without requiring hover")
