...
static func for_driver(sim: RaceSim, id: int, policy: Dictionary, forecast: Dictionary) -> Array:
	...
	if sim.phase != "race": return finalize(cards, policy, key)
	...
	if c.route == "pit" or c.pit_order: return finalize(cards, policy, key)
	if forecast.is_empty() or RaceForecaster.stale(sim, forecast, int(policy.revision)): return finalize(cards, policy, key)
	...
	return finalize(cards, policy, key)

static func finalize(cards: Array, policy: Dictionary, key: String) -> Array:
	for entry in cards:
		entry.acknowledged = policy.held.get(entry.issue, "") == key
