...
func _ready() -> void:
	...
	footer.add_child(UI.button("Export this scenario", export_scenario))
...
func start_sandbox() -> void:
	...
	var record = RaceRecord.new(); record.attach(sim, "sandbox", player.lineage())
...
func export_scenario() -> void:
	...
	if sim == null: return
	var record = RaceRecord.new(); record.attach(sim, "sandbox", player.lineage())
	export_data(record.seal(), "decision.scenario.json")
