...
static func describe(root: Node) -> void:
	if root is Control:
		...
		if root is LineEdit and root.get_parent() is SpinBox and root.accessibility_name.is_empty():
			root.accessibility_name = root.get_parent().accessibility_name if not root.get_parent().accessibility_name.is_empty() else root.get_parent().tooltip_text
		if root is Label and root.focus_mode != Control.FOCUS_ALL: root.focus_mode = Control.FOCUS_ACCESSIBILITY
