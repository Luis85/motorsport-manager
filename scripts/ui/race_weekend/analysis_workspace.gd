...
func _ready() -> void:
	...
	for id in [3,6]:
		var b=UI.button("",func():driver_requested.emit(id));b.size_flags_horizontal=Control.SIZE_EXPAND_FILL;team.add_child(b);drivers[id]=b
...
func present() -> void:
	...
	for id in drivers:
		var c=model.cars[id]
		drivers[id].text="%s · %s %.0f%% · finish fuel ~%+.1f · %s" % [c.short,c.compound,c.tyre,RaceForecaster.fuel_margin(model,c),c.intent]
