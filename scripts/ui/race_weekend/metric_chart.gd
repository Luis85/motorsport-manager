...
func _update_minimum() -> void:
	custom_minimum_size = Vector2(250, ceilf(182 * get_theme_font_size("font_size") / 13.0))
...
func present_samples(next_title: String, next_unit: String, values: Array, low: float, high: float, positions: Array, labels: Array = [], axis: String = "Elapsed s") -> void:
	_set_data(next_title, next_unit, values, low, high, [], "trace", positions, labels, axis)
...
func _set_data(next_title: String, next_unit: String, values: Array, low: float, high: float, labels: Array, next_mode: String, positions: Array, sample_labels: Array, axis: String) -> void:
	...
	if [title, unit, series, minimum, maximum, categories, mode, x_values, x_labels, domain] == [next_title, next_unit, safe_values, low, high, labels, next_mode, safe_positions, sample_labels, safe_axis]: return
...
func selected_text() -> String:
	...
	return "%s · %s" % [sample_label(cursor), "Unavailable" if series[cursor] == null else "%.2f %s" % [series[cursor], unit]]
...
func plot_area() -> Rect2:
	...
	return Rect2(52 * scale_factor, 40 * scale_factor, maxf(1, size.x - 66 * scale_factor), maxf(1, size.y - 107 * scale_factor))
...
func _draw() -> void:
	...
	if series.is_empty():
		...
		for i in range(series.size()):
			...
			if series.size() == 1 or i == cursor: draw_circle(point, 3, UI.PRIMARY)
			previous = point
		if cursor >= 0:
	...
	draw_string(font, Vector2(12, size.y - 25 * scale_factor), selected_text(), HORIZONTAL_ALIGNMENT_LEFT, size.x - 24, text_size, UI.INK)
